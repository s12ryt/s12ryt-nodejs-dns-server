...
- 候選`cd23e4e6c002201e494c875d317f891d623ada61`的GitHub CI run `31695856348`再次以29,998 ms被duration門檻拒絕，證明單次timer wait可提早喚醒。RED測試讓每次wait少推進2ms，現行碼使第二tick於18ms提前啟動；GREEN加入共用monotonic deadline迴圈，目標／鄰近測試10／10通過。完整回歸Node 231／231、Playwright 14／14、lint、audit 0與build通過，runtime digest維持`d375e7e476346b27e3c1a71c9caa081f2b9fb61efba9d216d231490369c17b55`。Linux glibc x64 scale運行30,002 ms，DNS 165,000次／0 error／5,499.71 QPS／p95 27 ms，proxy 33,000次／0 error／1,099.94 RPS／p95 20 ms，核心中斷0、維運1次／失敗0，evaluation通過。
- 候選`170e40eea239e77300a2445baba4b472626ab7ce`從`2026-08-13T11:47:01Z`執行Linux glibc x64 formal soak，約10小時20分後V8 heap達約2 GiB並以exit 134 OOM；主機仍有約7 GiB可用，無原子報告，判定失敗且不得拼接。按每tick 6,600個latency slots與37,252秒計算約1.97 GiB，定位為`batches`永久保留settled tick Promise及其閉包結果。
- 依使用者確認採只保留未完成批次。TDD RED因`InFlightBatches`不存在失敗；GREEN以Set追蹤Promise，成功或失敗settled後立即移除，`drain()`等待當下在途工作並保留第一個錯誤。目標／鄰近測試11／11；1,000,000個settled Promise後size 0，強制GC heap增量7,504 bytes。Linux glibc x64 scale運行30,002 ms，DNS 165,000次／0 error／5,499.61 QPS／p95 28 ms，proxy 33,000次／0 error／1,099.92 RPS／p95 19 ms，核心中斷0、維運1次／失敗0。完整回歸Node 232／232、Playwright 14／14、lint、audit 0與build均通過，runtime digest維持`d375e7e476346b27e3c1a71c9caa081f2b9fb61efba9d216d231490369c17b55`。
